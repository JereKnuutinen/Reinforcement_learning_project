...
class PG(object):
    ...
    def update(self,):
        ...
        # Hints:
        #   1. compute discounted rewards (use the discount_rewards function 
        #      offered in common.helper)
        #   2. compute the policy gradient loss
        #   3. update the parameters (backpropagate gradients, 
        #      do the optimizer step, empty optimizer 
        #      gradients afterwards so that gradients 
        #      don't accumulate over updates)
        
        # Discauted rewards
        #print(np.shape(action_probs))
    ...
    def get_action(self, observation, evaluation=False):
        ...
        ########## Your code starts here. ###########
        # Hint: 
        #   1. when evaluation=True, return mean, otherwise 
        #      return samples from the distribution created in 
        #      self.policy.forward() function.
        #   2. notice the shape of action and act_logprob.
        
        action = 0
        act_logprob = 0

        nd = self.policy.forward(x)
        if evaluation:
            #action = self.policy.actor_mean(x)
            action = nd.mean
            act_logprob = nd.log_prob(action)
        else:
            action = nd.sample((1,))[0]
            act_logprob = nd.log_prob(action)
