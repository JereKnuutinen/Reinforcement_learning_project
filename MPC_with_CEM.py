        ...
        with Parallel(n_jobs=-1,) as parallel:  # we use joblib.Parallel to parallel the evaluation.
            ...
            for _ in range(self.iteration):
                ...
                for ii in range(self.num_samples):
                    ...
                    G = rollout_simulator(model, actions)
                    values.append(G)
                
                '''
                trj = [np.random.normal(mean, std) for sample in range(self.num_samples)]
                #trj = np.swapaxes(trj, 0, 1)
                #print(np.shape(trj))
                #G = parallel(delayed(rollout_simulator(self.model, trj)))
                G = parallel(delayed(rollout_simulator)(model, action) for action in trj)
                #print("prööt")
                trj = np.asarray(trj)
                G = np.asarray(G)

                top_idx = np.argsort(G)[::-1][0:self.num_topk]
                
                _mean, _std = np.mean(trj[top_idx],axis=0), np.std(trj[top_idx], axis=0) # change this line 
...
if __name__ == "__main__":
    ...
    if args.use_wandb:
        ...
        wandb.init(project="rl_aalto",
                    name=f'hopper-{run_id}',
                    group=f'hopper')

    #env = make_env(env_name='hopper_medium',
     #               seed=1,
      #              action_repeat=6)

    env = create_env(config_file_name="hopper_medium", seed=1)
